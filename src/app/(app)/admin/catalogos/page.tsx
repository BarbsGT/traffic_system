...
export default function CatalogosPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [{ data: ta }, { data: aa }] = await Promise.all([
    ...
    const accountGrouped: Record<string, string[]> = {};
    if (aa) {
      aa.forEach((r) => {
        accountGrouped[r.account_id] = accountGrouped[r.account_id] || [];
        accountGrouped[r.account_id].push(r.agency_id);
      });
    }
    setAccountAssignments(
      (ac.data || []).map((account) => ({
        ...
        agency_ids: accountGrouped[account.id] || [],
      }))
    );
    setPage(0);
  }, [supabase]);
  ...
  const openEdit = (item: unknown) => {
    ...
    } else if (tab === "account-assignments") {
  ...
  return (
    <div className="animate-fadeIn">
        ...
        <table className="w-full">
          <thead>
            <tr style={{ background: "var(--table-header)" }}>
              ...
              {tab === "account-assignments" && <Headers cols={["Cuenta", "Agencias Asignadas", "Acción"]} />}
          ...
          <tbody>
            ...
            {tab === "account-assignments" && accountAssignments.map((asgn) => (
              <Row key={asgn.account_id} item={asgn} onEdit={() => setEditing(asgn)} onDelete={() => {}}>
                <td className="p-3 text-sm" style={{ color: "var(--text-primary)" }}>{asgn.account_name}</td>
                <td className="p-3 text-sm" style={{ color: "var(--text-secondary)" }}>
                  {asgn.agency_ids.length === 0
                    ? "Sin agencias"
                    : agencies.filter((a) => asgn.agency_ids.includes(a.id)).map((a) => a.name).join(", ")}
                </td>
                <td className="p-3 text-sm">
                  <button onClick={() => setEditing(asgn)}
                    className="px-3 py-1 rounded text-xs font-medium text-[#690003]"
      ...
      <Modal isOpen={showAccountAssignmentModal} onClose={() => setShowAccountAssignmentModal(false)} title={editing ? `Editar Asignación: ${(editing as AccountAssignment).account_name}` : "Nueva Asignación Cuenta → Agencias"}>
        <AccountAssignmentForm
          editing={editing as AccountAssignment | null}
          accounts={accounts}
          agencies={agencies}
          onDone={() => { setShowAccountAssignmentModal(false); setEditing(null); fetchData(); }}
        />
      </Modal>
...
function AccountAssignmentForm({ editing, accounts, agencies, onDone }: {
  editing: AccountAssignment | null;
  accounts: Account[];
  agencies: Agency[];
  onDone: () => void;
}) {
  ...
  useEffect(() => {
    if (editing) {
      setSelectedAccountId(editing.account_id);
      setSelectedAgencyIds(editing.agency_ids || []);
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const existing = await supabase.from("account_agencies").select("agency_id").eq("account_id", selectedAccountId);
    const existingAgencyIds = (existing.data || []).map((r) => r.agency_id);

    const toRemove = existingAgencyIds.filter((id) => !selectedAgencyIds.includes(id));
    const toAdd = selectedAgencyIds.filter((id) => !existingAgencyIds.includes(id));

    if (toRemove.length > 0) {
      await supabase.from("account_agencies").delete().eq("account_id", selectedAccountId).in("agency_id", toRemove);
    }

    if (toAdd.length > 0) {
      const rows = toAdd.map((agency_id) => ({ account_id: selectedAccountId, agency_id }));
      await supabase.from("account_agencies").insert(rows);
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1">
        <label className="text-xs font-medium" style={{ color: "var(--text-secondary)" }}>Agencias asignadas</label>
        <div className="flex flex-wrap gap-2" style={{ maxHeight: 200, overflow: "auto" }}>
          {agencies.map((a) => (
            <label key={a.id} className="flex items-center gap-2 px-3 py-2 rounded border cursor-pointer"
              style={{ background: "var(--input-bg)", borderColor: "var(--input-border)", color: "var(--text-primary)" }}>
              <input type="checkbox" value={a.id} checked={selectedAgencyIds.includes(a.id)}
                onChange={(e) => setSelectedAgencyIds((prev) =>
                  e.target.checked ? [...prev, a.id] : prev.filter((id) => id !== a.id)
                )}
                className="rounded" />
              <span className="text-sm">{a.name}</span>
